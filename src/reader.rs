...
impl Reader {
    pub fn new(file_name: &str) -> Self {
        let file = std::fs::File::open(file_name).expect(&format!("File: {file_name} could not be found"));
    ...
    pub fn next_indexed(&mut self) -> Option<(String, usize)> {
        loop {
            ...
            let string = line.expect(&format!("Line {} was not a string", index + 1)).trim().to_string();
